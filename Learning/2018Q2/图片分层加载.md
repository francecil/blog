...
浏览器 ->> shark:请求原图像资源
shark ->> shark:根据用户网速判断是否继续压缩图像
Note right of shark:缓存失效会继续请求源站
shark -->>浏览器:返回原图
```
...
graph TB
  st(开始)-->init((初始化))
  init-->obs[注册window.load事件监听器]
  obs-->isload{window.load?}
  isload--false-->isload
  isload--true-->cal[计算用户网络状况]
  cal-->scan((文档扫描url替换))
  scan-->stop(结束)
  init-.-evalElement(解析script标签中自定义属性)
  evalElement-->company[根据属性值初始化客户功能开启项]
  company-->setHost[设置可替换图片域名/散列域名/懒加载策略]
  setHost-->initStop(初始化结束)
  scan-.-scanInit(扫描文档)
  scanInit-->recordDom[记录style*background,cssDiv,img,picture等节点]
  recordDom-->islazyLoad{是否懒加载}
  islazyLoad--true-->sort[按视距快排节点]
  sort-->lazyload[启动懒加载策略]
  lazyload-->replace
  islazyLoad--false-->replace[找到满足条件节点,准备替换图片节点的url]
  replace-->ismedia{是否为picture节点或含srcset属性节点}
  ismedia--true-->reg[特殊处理,需要进行正则替换图片地址]
  reg-->isneedre{是否为可替换图片域名}
  ismedia--false-->isneedre
  isneedre--true-->seturl[替换图片节点的url,带上网络状况标识]
  seturl-->imgreq[浏览器发起图片请求]
  imgreq-->scanStop(图片url替换结束)
  isneedre--false-->scanStop
```
...
解决方案：这边我们主要就是判断图片是否为无缓存请求，当为无缓存请求，img.onload后需要对localstorage进行更新。至于怎么判断是否为无缓存，就用`performance.getEntriesByName('模糊图url').transferSize===0`来判断
